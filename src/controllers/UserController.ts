...
export default class UserController {
  ...
  public async favorites (req: Request, res: Response) {
    const userID = String(usersService.getUserID(res));
    let { search, cursor, limit } = req.query as { search?: string, cursor?: string, limit?: number };
    ...
    await Promise.all([
      words = await FavoriteModel
        .find(filter)
        .limit(limit) // limit the number of documents
        .sort({ word: 'asc' }),
      firstDocument = await FavoriteModel
        .findOne(filter)
        .limit(limit)
        .sort({ word: 'asc' }),
      lastDocument = await FavoriteModel
        .findOne(filter)
        .sort({ _id: -1, word: 'asc' }),
      totalOfDocuments = await FavoriteModel
        .countDocuments(filter)
    ]);
